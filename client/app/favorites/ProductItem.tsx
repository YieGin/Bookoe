...
const ProductItem: React.FC<ProductTypes> = ({ item }) => {
  ...
  return (
    <div className='border-b-[1px] dark:border-[#333333] md:p-5 xs:p-3 flex xs:w-full md:w-[700px] lg-md:w-[45%]'>
      <img onClick={() => navigateToProductPage(item.id)} className='md:w-40 md:h-40 xs:w-28 xs:h-36 rounded-[14px] object-cover mr-3 cursor-pointer' src={`${process.env.NEXT_PUBLIC_HOST}${item.image}`} alt={item.title} />
