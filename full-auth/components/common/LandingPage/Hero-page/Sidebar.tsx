...
import { fetchBestSellers } from '@/redux/features/productsSlice';
...
const Sidebar = () => {
  ...
  const handleNext = () => {
    setCurrentBestSellerIndex((prevIndex) => (prevIndex + 1) % 4);
  };

  const handlePrevious = () => {
    setCurrentBestSellerIndex((prevIndex) => (prevIndex - 1 + 4) % 4);
  };

  const currentBestSeller = bestSellers[currentBestSellerIndex] || {};

  return (
    <div className='bg-[#9C9C9C] dark:bg-[#232323] rounded-3xl h-[662px] md:flex xs:hidden items-center flex-col py-10 px-5'>
      ...
      <div className='flex justify-center items-center'>
        ...
        {bestSellers.length > 0 && (
          <div className="w-[200px] h-[300px] border-4 cursor-pointer border-white mt-5 shadow-white rounded-lg overflow-hidden hover:shadow-2xl transition-all duration-300 ease-in-out">
            <img 
